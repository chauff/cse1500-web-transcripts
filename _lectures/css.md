...
:point_up: In this example, we create a number of global CSS variables, i.e. variables available to all elements in the DOM tree. For this reason, we make use of the pseudo-class `:root` which represents the `<html>` element. Variables are defined with the custom prefix `--` and can be accessed using the `var()` function. Non-global CSS variables can be added in the same manner to any element, though they are then only available within their `{....}` block.
